    ...
    if (typeof dataOut[key] == 'string')
      ...
      if (Array.isArray(dataOut[key])) {
        ...
        for (let i = 0; i < dataOut[key].length; i++) {
          result[key].push(
            assignVariable(variableObject, { here: dataOut[key][i] })['here'],
          );
        }
      } else {
...
let actionData = {
  // get this from the action.data, one of depth only (not supported yet for nested objects)
...
let reactionData = {
  // get this from the reaction.data
  prompt: {
    title: ['__body__', '__cc__'],
    message: '__subject__',
  },
};
